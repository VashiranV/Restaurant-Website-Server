...
function selectRestaurant(){
	...
	if(result){
		...
		info.innerHTML = `<p style="font-family:sans-serif"><b><u>` +currentRestaurant.name + "</b></u><br>Minimum Order: $" + currentRestaurant.min_order + "<br>Delivery Fee: $" + currentRestaurant.delivery_fee + "<br><br>";
	}else{
...
function getCategoryHTML(rest){
	let menu = rest.menu;
	let result = `<p style="font-family:sans-serif"><b>Categories<b><br>`;
...
function getMenuHTML(rest){
	...
	Object.keys(menu).forEach(key =>{
		result += `<u><font size="+2"><p style="font-family:sans-serif"><b>${key}</b><a name="${key}"></a><br></font></u>`;
		//For each menu item in the category
		Object.keys(menu[key]).forEach(id => {
			item = menu[key][id];
			result += `<b>${item.name} (\$${item.price})</b> <img src='add.png' style='height:20px;vertical-align:bottom;' onclick='addItem(${id})'/> <br>`;
			result += "<i>"+item.description + "<br><br></i>";
		});
	});
	return result;
}
